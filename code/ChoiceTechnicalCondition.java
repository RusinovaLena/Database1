...
import net.codejava.ChoiceTypeEquipment.CustomerItem;
import net.codejava.ChoiceTypeEquipment.RenderCheckComboBox;

public class ChoiceTechnicalCondition implements ActionListener {
	
	static CustomerItem[] typeCondition;	
	...
	public JComboBox<CustomerItem> outputPanel(ArrayList headers, String firstString) {
		 ...
		 JComboBox<CustomerItem> combo = new JComboBox<CustomerItem>() {
	        ...
	        typeCondition = new CustomerItem[headers.size() + 1];
	        typeCondition[0] = new CustomerItem(firstString, true);
	        for (int i = 0; i < headers.size(); i++) {
	        	typeCondition[i + 1] = new CustomerItem(headers.get(i).toString(), false);
	        }	
	        
	        combo.setModel(new DefaultComboBoxModel<CustomerItem>(typeCondition));
	        ...
	        combo.addActionListener(new ActionListener() {
	            ...
	            public void actionPerformed(ActionEvent ae) {
	                ...
	                for (int i = 1; i < typeCondition.length; i++) {
	                	if (typeCondition[i].status == true) {
	                		typeCondition[0].status = false;
	                	}
	                }	
	            }
	        });
	...
	public ArrayList selectedTypes() {
    	...
    	for (int i = 0; i < typeCondition.length; i++) {
    		if (typeCondition[i].status == true) {
    			selectedValues.add(typeCondition[i].label);
    		}	
    	}
