...
public class ChoiceAppointment implements ActionListener {
	
	static CustomerItem[] appointment;	
	...
	public JComboBox<CustomerItem> outputPanel(ArrayList headers, String firstString) {	
		 ...
		 JComboBox<CustomerItem> combo = new JComboBox<CustomerItem>() {
	        ...
	        appointment = new CustomerItem[headers.size() + 1];
	        appointment[0] = new CustomerItem(firstString, true);
	        for (int i = 0; i < headers.size(); i++) {
	        	appointment[i + 1] = new CustomerItem(headers.get(i).toString(), false);
	        	
	        }	
	        
	        combo.setModel(new DefaultComboBoxModel<CustomerItem>(appointment));
	        ...
	        combo.addActionListener(new ActionListener() {
	            ...
	            public void actionPerformed(ActionEvent ae) {
	                ...
	                for (int i = 1; i < appointment.length; i++) {
	                	if (appointment[i].status == true) {
	                		appointment[0].status = false;
	                	}
	                }	
	            }
	        });
	...
	public ArrayList selecting() {
		
		ArrayList selectedValues = new ArrayList<Integer>();
			
    	for (int i = 0; i < appointment.length; i++) {
    		if (appointment[i].status == true) {
    			selectedValues.add(appointment[i].label);
    		}	
    	}
