...
public class ChoiceLocation implements ActionListener {
	
	static CustomerItem[] locations;	
	...
	public JComboBox<CustomerItem> outputPanel(ArrayList headers, String firstString) {	
		 ...
		 JComboBox<CustomerItem> combo = new JComboBox<CustomerItem>() {
	        ...
	        locations = new CustomerItem[headers.size() + 1];
	        locations[0] = new CustomerItem(firstString, true);
	        for (int i = 0; i < headers.size(); i++) {
	        	locations[i + 1] = new CustomerItem(headers.get(i).toString(), false);
	        	
	        }	
	        
	        combo.setModel(new DefaultComboBoxModel<CustomerItem>(locations));
	        ...
	        combo.addActionListener(new ActionListener() {
	            ...
	            public void actionPerformed(ActionEvent ae) {
	                ...
	                for (int i = 1; i < locations.length; i++) {
	                	if (locations[i].status == true) {
	                		locations[0].status = false;
	                	}
	                }	
	            }
	        });
	...
	public ArrayList selectedLocations() {
    	...
    	for (int i = 0; i < locations.length; i++) {
    		if (locations[i].status == true) {
    			selectedValues.add(locations[i].label);
    		}	
    	}
