...
public class ChoiceEquipmentOwners implements ActionListener {
	
	static CustomerItem[] equipmentOwners;	
	...
	public JComboBox<CustomerItem> outputPanel(ArrayList headers, String firstString) {
		 ...
		 JComboBox<CustomerItem> combo = new JComboBox<CustomerItem>() {
	        ...
	        equipmentOwners = new CustomerItem[headers.size() + 1];
	        equipmentOwners[0] = new CustomerItem(firstString, true);
	        for (int i = 0; i < headers.size(); i++) {
	        	equipmentOwners[i + 1] = new CustomerItem(headers.get(i).toString(), false);
	        }	
	        
	        combo.setModel(new DefaultComboBoxModel<CustomerItem>(equipmentOwners));
	        ...
	        combo.addActionListener(new ActionListener() {
	            ...
	            public void actionPerformed(ActionEvent ae) {
	                ...
	                for (int i = 1; i < equipmentOwners.length; i++) {
	                	if (equipmentOwners[i].status == true) {
	                		equipmentOwners[0].status = false;
	                	}
	                }	
	            }
	        });
	...
	public ArrayList selectedHeaders() {
    	...
    	for (int i = 0; i < equipmentOwners.length; i++) {
    		if (equipmentOwners[i].status == true) {
    			selectedValues.add(equipmentOwners[i].label);
    		}	
    	}
