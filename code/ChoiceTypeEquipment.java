...
import net.codejava.ChoiceEquipmentOwners.CustomerItem;
import net.codejava.ChoiceEquipmentOwners.RenderCheckComboBox;

public class ChoiceTypeEquipment implements ActionListener {
	
	static CustomerItem[] typeEquipment;	
	...
	public JComboBox<CustomerItem> outputPanel(ArrayList headers, String firstString) {
		 ...
		 JComboBox<CustomerItem> combo = new JComboBox<CustomerItem>() {
	        ...
	        typeEquipment = new CustomerItem[headers.size() + 1];
	        typeEquipment[0] = new CustomerItem(firstString, true);
	        for (int i = 0; i < headers.size(); i++) {
	        	typeEquipment[i + 1] = new CustomerItem(headers.get(i).toString(), false);
	        }	
	        
	        combo.setModel(new DefaultComboBoxModel<CustomerItem>(typeEquipment));
	        ...
	        combo.addActionListener(new ActionListener() {
	            ...
	            public void actionPerformed(ActionEvent ae) {
	                ...
	                for (int i = 1; i < typeEquipment.length; i++) {
	                	if (typeEquipment[i].status == true) {
	                		typeEquipment[0].status = false;
	                	}
	                }	
	            }
	        });
	...
	public ArrayList selectedTypes() {
    	...
    	for (int i = 0; i < typeEquipment.length; i++) {
    		if (typeEquipment[i].status == true) {
    			selectedValues.add(typeEquipment[i].label);
    		}	
    	}
